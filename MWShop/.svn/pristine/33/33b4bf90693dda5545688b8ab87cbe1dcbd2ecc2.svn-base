		...
		<ul>
			...
			<li>
				<dl>
					...
					<!--当前链接则添加class:active-->
					<dd>
						<a href="/MWShop/AProductController/selectAllProduct">分类修改</a>
					</dd>
				</dl>
			</li>
	...
	<section class="rt_wrap content mCustomScrollbar">
		<div class="rt_content">
			...
			<section>
				...
				<form
					action="${pageContext.request.contextPath}/AControllerSecond/editCategorySecond?csid=${csid}" method="post">
					<ul class="ulColumn2">
					<li><span class="item_name" style="width: 120px;">分类:</span>
							...
							<select class="select" name="cid">
								...
								<c:forEach items="${categroys}" var="categroys">
									<option value="${categroys.cid}" selected>${categroys.cname}</option>
								</c:forEach>
						</select></li>
						<c:forEach items="${categorySecondList }" var="categorySecondList" >
						<li><span class="item_name" style="width: 120px;">名称：</span>
							<input type="text" class="textbox" placeholder="输入二级分类名称"name="csname" value="${categorySecondList.csname}" /></li></c:forEach>
